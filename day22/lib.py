...
def shuffle(actions: list[list[str]], number_of_cards: int) -> tuple[int, int]:
    scale = 1
    shift = 0
    for row in actions:
        if row[1] == 'into':
...
def repeat(scale: int, shift: int, number_of_shuffles: int, number_of_cards: int) \
        -> tuple[int, int]:
